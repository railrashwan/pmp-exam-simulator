...
async function main() {
  ...
  if (files.length === 0) {
    console.error("Usage: npx tsx prisma/seed-helena.ts <file1.txt> [file2.txt]");
    process.exit(1);
  }

  // First, clear any existing helena records
  await prisma.question.deleteMany({
    where: { examSet: "helena" },
  });
  console.log("Cleared existing Helena questions.");
